...
# Guarded import: commentary_engine pulls in heavy/optional deps. If it fails to
# import (missing dependency, wrong file content, etc.) the WHOLE web service
# must NOT crash on boot — only /api/commentary degrades. A bare top-level
# import here once silently failed every deploy and left a stale build live.
try:
...
@app.route("/")
def home():
    # Endpoints are INTROSPECTED from the live URL map, never hand-typed.
    # A hand-maintained list silently went stale — it advertised routes that
    # didn't exist and hid ones that did, which made debugging deploys a
    # nightmare. This list is now guaranteed to match the routes actually
    # registered on the running build.
    endpoints = sorted(
        str(rule)
...
def get_market_commentary():
    try:
        ...
        # NOTE: post-market commentary is generated by the cron at 17:00 IST,
        # so 'latest' must not flip to "post" until 17:00 — otherwise the
        # 16:00–17:00 window would serve YESTERDAY's post (stale) instead of
        # today's final intraday (15:30) slot.
        if ctype == "latest":
            if hour_min >= 17 * 60:           # 17:00 IST onwards
                ctype = "post"
            elif hour_min >= 9 * 60 + 15:     # 09:15 - 16:59 IST
                ctype = "intraday"
        ...
        if not result.data:
            ...
            if not fallback_q.data:
                msg = {
                    ...
                    "post":     "No post-market commentary yet. Generated daily at 17:00 IST.",
                }[ctype]
