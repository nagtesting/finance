...
(function addFineTuneInputs() {
  ...
  // Run on DOM ready — always deferred via setTimeout(0) so that
  // DOMContentLoaded listeners in the main script (setMode, navTo)
  // execute first, ensuring the SIP tab layout is stable before
  // ft-wrap flex wrappers are injected.
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      setTimeout(initFineTuneAll, 0);
    });
  } else {
    setTimeout(initFineTuneAll, 0);
  }
